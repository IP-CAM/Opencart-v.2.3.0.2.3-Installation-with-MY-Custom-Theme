			...
			<table>
				...
				<?php foreach ($products as $key => $product) { ?>
					<tr class="row_<?php echo $key; ?>_<?php echo $product['id']; ?>">
						...
						<td class="remove"><i class="fas fa-trash-alt" title="<?php echo $button_remove; ?>" onclick="$(this).parent().prev().prev().find('input').val(''); updateCart('<?php echo $product['id']; ?>', '<?php echo $product['key']; ?>')" ></i></td>
					</tr>
					<tr class="hr"><td colspan="6"></td></tr>
				<?php } ?>
			</table>
		...
		<div class="popupcart_total">
			<table>
				<?php foreach($totals as $total) { ?>
					<tr>
					 <td class="right"><?php echo $total['text']; ?></td><td class="right"><?php echo $total['title']; ?>: </td>
					</tr>
				<?php } ?>
			</table>
		</div>
