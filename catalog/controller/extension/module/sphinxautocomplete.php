...
class  ControllerExtensionModuleSphinxautocomplete extends Controller {
	...
	public function index() {
		...
		foreach ($resultsProducts['results'] as $product) {
		    $image = $this->model_tool_image->resize(($product['image']) ? $product['image'] : 'no_image.jpg', 50, 50);
        if ($product['special']) {
        $special = $this->currency->format($product['special'], $this->session->data['currency']);
        $pricespec  = $this->currency->format($product['price'], $this->session->data['currency']);
        $price = $special . '<span>' . $pricespec . '</span>';
        } else {
	...
	$resultsManufacturers = $this->model_catalog_sphinx->search($searchData, 'manufacturers', true);

		foreach ($resultsManufacturers['results'] as $manufacturer) {
		...
		$this->response->setOutput(json_encode(array('products' => $products, 'manufacturers' => $manufacturers, 'categories' => $categories)));
    }
