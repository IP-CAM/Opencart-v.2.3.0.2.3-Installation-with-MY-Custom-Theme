<?php
//Переклад: Том'як Олег з любов'ю до Української мови та легкості Opencart
// Heading
$_['heading_title']     = 'Порівняння товарів';

// Text
$_['text_product']      = 'Інформація про товар';
$_['text_name']         = 'Найменування';
$_['text_image']        = 'Зображення';
$_['text_price']        = 'Ціна';
$_['text_model']        = 'Модель';
$_['text_manufacturer'] = 'Виробник';
$_['text_availability'] = 'Наявність';
$_['text_instock']      = 'В наявності';
$_['text_rating']       = 'Рейтинг';
$_['text_reviews']      = 'На підставі %s відкликання(ів).';
$_['text_summary']      = 'Всього';
$_['text_weight']       = 'Вага';
$_['text_dimension']    = '(Д x Ш x В)';
$_['text_compare']      = 'Порівняння товарів (%s)';
$_['text_success']      = '<a href="%s"><img src="%s" class="img-thumbnail" width="50" height="50" alt=""/>%s </a> доданий до <a href="%s"> Списку порівняння </a>!';
$_['text_remove']       = 'Перелік товарів успішно оновлено!';
$_['text_empty']        = 'Оберіть будь ласка товари для порівняння';