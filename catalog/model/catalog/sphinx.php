...
class ModelCatalogSphinx extends Model
{
 ...
 public function getManufacturersForFilter($manufacturerId)
    {
        $this->load->model('catalog/manufacturer');

        $ids = array();

        $manufacturers = $this->model_catalog_manufacturer->getManufacturers($manufacturerId);

        foreach ($manufacturers as $manufacturer) {
            $ids[] = $category['manufacturer_id'];

            $ids = array_merge($ids,$this->getManufacturerForFilter($manufacturer['manufacturer_id']));
        }
    ...
    public function search($data, $index = null, $wildcard = false)
    {
        ...
        foreach ($searchResults['matches'] as $docinfo) {
            ...
            else if ($index == 'manufacturers'){
                $record = $this->model_catalog_manufacturer->getManufacturer((int) $id);                
            } else {
